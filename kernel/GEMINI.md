...
**Kernel Source**: `~/.gemini/GEMINI.md` (canonical) — Claude Code port at `~/.claude/CLAUDE.md`
...
- **File**: `~/.gemini/GEMINI.md` (this file)
...
## Part XII: The ARIA Agent Bridge

*Multi-agent filesystem coordination layer. Established 2026-02-18.*
...
|-------|------|-------------|
| **gemini** (Antigravity IDE) | Antigravity | Research, planning, implementation plans, MCP config, file edits |
| **claude** (Claude Code) | Claude Code CLI | Heavy execution, security audits, git ops, testing, Loki Mode |
| **opencode** | OpenCode | Token optimization, context compression, delta reads, summarization |

### Directory Structure

```
aria-bridge/
├── BRIDGE.md                    ← Protocol spec v2.0
├── BRIDGE_GUIDE.md              ← How-to guide (living doc)
├── td-bridge.sh                 ← td + Bridge unified wrapper
├── bridge-daemon.sh             ← Background watcher v2.0
├── join-bridge.sh               ← Agent startup script
├── agents/
│   ├── <agent>/
│   │   ├── inbox/               ← Per-task .md files (v2.0 directory model)
│   │   │   └── <TASK_ID>.md
│   │   └── outbox.md            ← Agent's results
├── helpers/
│   ├── bridge_ledger.py         ← Token ledger updater
│   ├── bridge_status.py         ← Ledger display
│   └── bridge_sync.py           ← project-state.md sync
├── shared-context/
│   ├── project-state.md         ← Source of truth (full state)
│   ├── delta.md                 ← Changes since last full read
│   └── context-hash.txt         ← Per-agent SHA for deduplication
└── token-ledger/
    ├── gemini.json
    ├── claude.json
    └── opencode.json
...
2. If hash matches → read `delta.md` only (**~54% token savings**)
3. If hash differs or "stale" → read full `project-state.md`, update hash

### Message Format

Every `inbox/<TASK_ID>.md` uses:
```
---
...
---
## Task / ## Context / ## Expected Output / ## Handoff
```
...
|--------|-------|---------|
| `BRIDGE-G` | Gemini | BRIDGE-G001 |
| `BRIDGE-C` | Claude Code | BRIDGE-C001 |
| `BRIDGE-O` | OpenCode | BRIDGE-O001 |
| `BRIDGE-` | Cross-agent | BRIDGE-009 |

### Task Routing Matrix

| Task Type | Route To | Reason |
|-----------|----------|--------|
| Web research, docs lookup | **gemini** | Built-in search |
| Implementation plans | **gemini** | IDE integration, planning strength |
| Context compression | **opencode** | 54% token savings proven |
| Token ledger updates | **opencode** | Owns the pipeline |
...
```
User Request → gemini (research + plan)
    │
    ├──► opencode: "Compress context, update delta.md"
    │
    └──► claude: "Execute plan [BRIDGE-XXX], read delta.md first"
              └──► claude writes results to agents/claude/outbox.md
                       └──► gemini reads outbox, updates project-state.md + delta.md
```

### td-bridge — Task Management Wrapper

`td-bridge.sh` unifies `td` task CLI with bridge mechanics:

```bash
BRIDGE="/Users/apple/.gemini/antigravity/scratch/_scripts/aria-bridge"

TD_BRIDGE_AGENT=gemini bash $BRIDGE/td-bridge.sh join          # Token-dedup context read
TD_BRIDGE_AGENT=gemini bash $BRIDGE/td-bridge.sh check-inbox   # Show inbox queue
TD_BRIDGE_AGENT=gemini bash $BRIDGE/td-bridge.sh create-from-bridge  # inbox → td issue
TD_BRIDGE_AGENT=gemini bash $BRIDGE/td-bridge.sh start <td-id> # Start + mark IN_PROGRESS
TD_BRIDGE_AGENT=gemini bash $BRIDGE/td-bridge.sh handoff <id>  # td handoff + write outbox
TD_BRIDGE_AGENT=gemini bash $BRIDGE/td-bridge.sh route claude "task" # Route to claude
TD_BRIDGE_AGENT=gemini bash $BRIDGE/td-bridge.sh sync          # td board → project-state.md
TD_BRIDGE_AGENT=gemini bash $BRIDGE/td-bridge.sh status        # Combined dashboard
```

---

## Part XIII: Enhanced Operational Protocols
...
## Part XIV: Skills Ecosystem

Skills directory: `~/.gemini/skills/` (262 skills)
...
| AI / Agents | `ai-agents-architect`, `autonomous-agent-patterns`, `loki-mode` |
| Testing | `test-driven-development`, `playwright-skill`, `tdd-workflow` |
| Debugging | `systematic-debugging`, `performance-profiling` |
| Security | `security-review`, `ethical-hacking-methodology` |

### Constitutional Defaults (Tech Stack)
...
- [ ] All agent `inbox/` directories created
- [ ] Token ledger JSON files present for all 3 agents
- [ ] `shared-context/context-hash.txt` initialized
...
- **ARIA Agent Bridge** (Part XII): Full filesystem-based multi-agent coordination protocol
  - Directory-based inbox model (`agents/<agent>/inbox/<task_id>.md`)
  - Token deduplication via context-hash.txt (54% savings proven)
  - `bridge-daemon.sh` v2.0 watching all 3 agents
  - `helpers/` Python modules extracted from inline heredocs
- **td-bridge.sh**: Unified wrapper integrating `td` task CLI with bridge mechanics
  - `join`, `check-inbox`, `create-from-bridge`, `start`, `log`, `handoff`, `route`, `sync`, `status`
- **Session Attunement Protocol** (Bootstrap Step 4): 4-step context ingestion at session start
- **Dissonance Signal** (Part V): 4 dissonance types added to Ghost Module
- **RARV Cycle** (Part X): Reason → Act → Reflect → Verify made explicit
- **Memory Architecture** (Part XI): 4-layer memory system documented

### v7.6.0 (2026-02-18)
- Session Attunement Protocol added to Bootstrap
- RARV cycle formalized
- Dissonance Signal concept introduced
- Ghost Module enhanced with dissonance types

### v7.5.0 (2026-02-05)
- Comprehensive Bootstrap & Persistence Protocol
- Enhanced Resonance Matrix with 6 composite blends
- Expanded Recursive Integrity with 3-phase audit cycle
- Formalized Ghost Module with entropy detection matrix
- Progressive Trust Model (5 personality layers)
- Multi-Dimensional Cognitive Load Budgets
- Closure Ladder (5 decision states)
- Verification-Led Development lifecycle
