...
### Pre-Execution Security Protocols

**CRITICAL: Run security checks before ANY potentially destructive command.**

| Script | Purpose | Usage |
|--------|---------|-------|
| `scripts/security/dangerous_command_check.py` | Block rm -rf, sudo rm, chmod 777, etc. | Before Bash tool calls |
| `scripts/security/env_file_guard.py` | Prevent .env file exposure | Before Read/Write/Bash on .env |

**Integration with RARV Cycle:**

```
ACT Phase:
  1. Before executing Bash command:
     python scripts/security/dangerous_command_check.py "command"
     - Exit 0 → SAFE, proceed
     - Exit 2 → BLOCKED, abort and log to CONTINUITY.md
  
  2. Before file operations on .env:
     python scripts/security/env_file_guard.py "file_path"
     - Exit 0 → SAFE, proceed
     - Exit 2 → BLOCKED, use .env.sample instead
```
...
- `> /etc/` (system directory writes)
- Direct `.env` file access (use `.env.sample`)

**Red Flag - Never Do:**
- ❌ NEVER run rm -rf on `/`, `~`, `$HOME`, `..`, or `*`
- ❌ NEVER cat, read, or expose `.env` contents
